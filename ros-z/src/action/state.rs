...
/// Actions that can be performed after releasing the lock.
///
/// This pattern separates state queries (done while holding the lock)
/// from I/O operations (done after releasing the lock).
pub enum Action<T> {
    SendResult(T),
    RegisterWaiter(GoalId),
    Ignore,
}
